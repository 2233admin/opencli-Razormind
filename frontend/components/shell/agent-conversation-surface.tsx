...
export function AgentConversationSurface({
  ...
  async function sendMessage(event?: FormEvent) {
    ...
    if (providersQuery.isSuccess && !modelConfigured) {
    ...
    <div
      className={presentation === 'dock'
        ...
        : presentation === 'dashboard'
          ...
          : 'flex min-h-[680px] flex-col overflow-hidden rounded-xl border bg-card shadow-sm'}
      aria-label={presentation === 'dock' ? '全局 Agent' : 'Ask Alice 主对话'}
    >
        <div className="border-b px-4 py-3">
          <h2 className="flex items-center gap-2 text-base font-semibold">
            <Bot className="size-4 text-primary" aria-hidden />
            {presentation === 'dock' ? '全局 Agent' : 'Ask Alice'}
          </h2>
        ...
        <ScrollArea className="min-h-0 flex-1">
          <div className="space-y-3 p-4" aria-live="polite">
            ...
            {providersQuery.isSuccess && !modelConfigured ? (
        ...
        <form className="border-t p-4" onSubmit={(event) => void sendMessage(event)}>
          <Textarea
            ...
            disabled={sending || confirming || selectedSession?.status === 'closed' || (providersQuery.isSuccess && !modelConfigured)}
          />
