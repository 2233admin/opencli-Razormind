...
def run_youtube_transcript_extractor_task(api_key, target_url):
    ...
    payload = {
        ...
        "input_parameters": [{"name": "TargetURL", "value": target_url}],
    }

    # 1. Start Task
    print(
        f"Starting YouTube Transcript Extractor task for URL: {target_url}",
        flush=True,
    )
    try:
        res = requests.post(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python youtube_transcript_extractor_api.py <target_url>",
            flush=True,
        )
        sys.exit(1)
